...
import { type MusicMetadata, icons } from "../Typings/music.js";
...
export default new MusicEvent("playerStart", (queue, track) => {
    ...
    if (metadata.isRadio) {
        if (metadata.interaction.channel !== null)
            metadata.client.RadioChannels.push(metadata.interaction.channel as VoiceBasedChannel);
