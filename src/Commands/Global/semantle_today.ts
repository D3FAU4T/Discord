...
export default new Command({
    name: "semantle_today",
    description: "Play a game of semantle with today's hidden word",
    emote: false,
    data: new SlashCommandBuilder()
    .setName("semantle_today")
    .setDescription("Play a game of semantle with today's hidden word"),
    run: async ({ interaction, client }) => {
        ...
        if (interaction.channel && interaction.guild) {
            ...
            await (channel as PartialTextBasedChannelFields).send({
                embeds: [
                    new EmbedBuilder()
                        .setTitle("D3mantle game initiation: Today's Word")
                        .setDescription("Game initiated, you can start guessing already!")
                        .setColor("Gold")
                ]
            });

            client.semantle[channel.id] = new Demantle(1);

            await interaction.editReply({
                embeds: [
                    new EmbedBuilder()
                        .setTitle("D3mantle: Today's Word")
