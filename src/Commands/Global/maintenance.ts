  ...
  run: async ({ client, interaction }) => {
    ...
    try {
      if (interaction.options.getSubcommand() === 'update_cheater_names') {
        ...
        const res = await client.functions.updateCheaterNames();

        if (res === undefined) await message.edit({
          embeds: [
            new EmbedBuilder()
          ...
          embeds: [
            new EmbedBuilder()
              ...
              .setDescription("Check if the bot Oauth is expired or not <:garlictv:1090616279112622122>")
              .setColor("Red")
          ]
        })
      }
